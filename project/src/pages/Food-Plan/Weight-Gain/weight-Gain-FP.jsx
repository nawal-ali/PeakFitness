...
const WeightGain = () => {
  ...
  const renderTextWithHighlight = (text, section) => {
    ...
    if (highlightMatch) {
      ...
      return (
        <>
          <span className="highlight">{highlightMatch}</span>
          {remainingText}
        </>
      );
    }
        ...
        <ol>
          {items.map((item, index) => {
            if (item === 'Carbohydrate is one of the important sources of energy, providing 4 calories per gram.') {
              return (
                <li key={index} className="no-number-white">
                  {item}
                </li>
              );
            }
        ...
        <>
          <ul>
            {items.map((item, index) => {
              ...
              return (
                <li key={index} className={isMealItem ? 'meal-item' : ''}>
          ...
          <h3 className="table-title">Animal-based protein sources</h3>
          ...
          <h3 className="table-title">Plant foods containing protein</h3>
          <table className="prot-table">
            <thead>
              <tr>
                <th>Food Source</th>
          ...
          <h3 className="table-title">Protein sources from nuts and seeds</h3>
          <table className="prot-table">
            <thead>
              <tr>
                <th>Food Source</th>
    ...
    if (section === 'Carbohydrates') {
      return (
        <>
          <ul>
            {items.map((item, index) => {
              if (item === 'Tips For Choosing Carbohydrates:') {
                return (
                  <li key={index} className="no-bullet-left">
                    {renderTextWithHighlight(item, section)}
                  </li>
                );
              }
          ...
          {/* جدول الكربوهيدرات */}
          <h3 className="table-title">Carbohydrate Content in Common Foods</h3>
          <table className="carb-table">
            <thead>
              <tr>
                <th>Food Source</th>
                <th>Quantity</th>
    ...
    <div className={`main-content ${isMounted ? 'mounted' : ''}`}>
      ...
      <aside className="left-panel">
        ...
        <div className="button-row">
          <button
            className={`nav-button ${activeSection === 'General Tips' ? 'active' : ''}`}
            onClick={() => setActiveSection('General Tips')}
          >
            General Tips
          </button>
          <button
            className={`nav-button ${activeSection === 'Healthy Fats' ? 'active' : ''}`}
        ...
        <div className="button-row">
          <button
            className={`nav-button ${activeSection === 'Protein' ? 'active' : ''}`}
            onClick={() => setActiveSection('Protein')}
          >
            Protein
          </button>
          <button
            className={`nav-button ${activeSection === 'Carbohydrates' ? 'active' : ''}`}
        ...
        <button
          className={`nav-button full-width ${activeSection === 'How To Calculate Calories' ? 'active' : ''}`}
          onClick={() => setActiveSection('How To Calculate Calories')}
        >
          How To Calculate Calories
        </button>
        <button
          className={`nav-button full-width ${activeSection === 'Carbohydrate Content In Common Foods' ? 'active' : ''}`}
