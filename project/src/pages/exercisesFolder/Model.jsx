...
// import './model.css';
// export default function Model({ onLabelClick }) {
...
export default function Model({ onLabelClick,setClickedOn }) {
  const handleClick = (label) => {
    setClickedOn(label);
    console.log("Clicked label from model:", label);
  };
  useEffect(() => {
    const modelViewer = document.querySelector('#hotspot-camera-view-demo');
  ...
  const hotspots = [
    ...
    { label: 'latissimus dorsi', position: '-0.3m 2.4m -0.15m', normal: '-0.3m 2.4m -0.15m', orbit: '-160deg 70deg 0.5m', target: '0m 2.5m 2m' },
